...
| 2026-07-29 | PR | Addressed integration feedback on DE, provenance, environment, and held-out membership | `629a2ba`, `f278e37` |`629a2ba`, `f278e37`, `6b082f4`, `30491848892` | Coefficient aliases resolve formulaic labels; fallback counts reflect applied events; pseudobulk count content is hashed; `[full]` declares `anndata>=0.11.4,<0.14` and `zarr>=2,<4`; `apply_pseudobulk_membership` is public and provenance-linked; local suite `112 passed`; final CI green for DE, pathway, and Python 3.10-3.12. |
| 2026-07-29 | PR | Fixed contained PyDESeq2 AnnData vector-shape compatibility | pending | Normalized one-dimensional `varm` and `obsm` fields through package-owned backing vectors while preserving public mirrors. Full synthetic fits pass for AnnData 0.11.4 + Zarr 2.18.7, AnnData 0.12.19 + Zarr 3.2.1, and AnnData 0.13.2 + Zarr 3.2.1; focused regression test `8 passed`; full suite validation follows. Metadata now declares `anndata>=0.11.4,<0.14` and `zarr>=2,<4`; the supported pair for AnnData 0.11 remains Zarr 2.
