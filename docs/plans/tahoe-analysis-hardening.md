...
| 2026-07-29 | PR | Addressed integration feedback on DE, provenance, environment, and held-out membership | `629a2ba`, `f278e37` |`629a2ba`, `f278e37`, `6b082f4`, `30491848892` | Coefficient aliases resolve formulaic labels; fallback counts reflect applied events; pseudobulk count content is hashed; `[full]` declares `anndata>=0.11.4,<0.12` and `zarr>=2,<3`; `apply_pseudobulk_membership` is public and provenance-linked; local suite `112 passed`; final CI green for DE, pathway, and Python 3.10-3.12. |
| 2026-07-29 | PR | Completed AnnData/Zarr co-dependency matrix | pending | Wheel metadata: AnnData 0.11.4 has a runtime major-version guard rejecting Zarr 3 and uses Zarr 2 APIs; AnnData 0.12.19 declares `zarr!=3.0.*,>=2.18.7`; AnnData 0.13.2 declares `zarr>=3.1`. Runtime checks: 0.11.4 + 2.18.7 passes the full local suite; 0.11.4 + 3.2.1 fails at import; 0.12.19 + 2.18.7 and +3.2.1 prepare but vendored PyDESeq2 fitting fails; 0.13.2 + 3.2.1 prepares but remains outside the supported fit stack. The full extra now declares `anndata>=0.11.4,<0.12` and `zarr>=2,<3`.
