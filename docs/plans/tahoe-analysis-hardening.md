...
| 2026-07-29 | PR | Fixed contained PyDESeq2 AnnData vector-shape compatibility and selected Python-conditional defaults | pending | Normalized one-dimensional `varm` and `obsm` fields through package-owned backing vectors while preserving public mirrors. Full synthetic fits pass for AnnData 0.11.4 + Zarr 2.18.7, AnnData 0.12.19 + Zarr 2.18.7, AnnData 0.12.19 + Zarr 3.2.1, and AnnData 0.13.2 + Zarr 3.2.1; focused regression test `8 passed`; full suite `113 passed` on Python 3.12. Metadata uses the legacy pair on Python 3.10 and `anndata>=0.12.19,<0.14` plus `zarr>=2.18.7,<4,!=3.0.*` on Python 3.11-3.14.
