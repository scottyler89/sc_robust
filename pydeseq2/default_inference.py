...
class DefaultInference(inference.Inference):
    ...
    def __init__(
        ...
        self.last_irls_diagnostics: list[dict[str, object]] = []
        self.last_alpha_mle_diagnostics: list[dict[str, object]] = []
    ...
    def irls(  # noqa: D102
        ...
        res = list(res)
        MLE_lfcs_, mu_hat_, hat_diagonals_, converged_, diagnostics = zip(*res)
        MLE_lfcs_ = np.array(MLE_lfcs_)
        mu_hat_ = np.array(mu_hat_)
        hat_diagonals_ = np.array(hat_diagonals_)
        converged_ = np.array(converged_)
        self.last_irls_diagnostics = list(diagnostics)
    ...
    def alpha_mle(  # noqa: D102
        ...
        res = list(res)
        dispersions_, l_bfgs_b_converged_, diagnostics = zip(*res)
        dispersions_ = np.array(dispersions_)
        l_bfgs_b_converged_ = np.array(l_bfgs_b_converged_)
        self.last_alpha_mle_diagnostics = list(diagnostics)
