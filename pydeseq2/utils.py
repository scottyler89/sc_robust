...
def irls_solver(
    ...
    diagnostics = {"irls_ridge_epsilon": 1e-6, "irls_ridge_applied": True}
    ...
    if return_diagnostics:
        return beta, mu, H, converged, diagnostics
...
def fit_alpha_mle(
    ...
    def dloss_ridge(log_alpha: float) -> float:
        alpha = np.exp(log_alpha)
        reg_grad = 0.0
        if cr_reg:
            W = mu / (1 + mu * alpha)
            dW = -(W**2)
            fisher = (design_matrix.T * W) @ design_matrix
            ridge_fisher = fisher + 1e-6 * np.eye(design_matrix.shape[1])
            reg_grad += (0.5 * (np.linalg.inv(ridge_fisher) * ((design_matrix.T * dW) @ design_matrix)).sum()) * alpha
        if prior_reg:
            if prior_disp_var is None:
                raise ValueError("Sigma_prior is required for prior regularization")
            reg_grad += (log_alpha - log_alpha_hat) / prior_disp_var
    ...
    bounds = [(np.log(min_disp), np.log(max_disp))] if optimizer == "L-BFGS-B" else None
    try:
        res = minimize(
            lambda x: loss(x[0]),
            x0=np.log(alpha_hat),
        ...
        cox_reid_ridge_retry = True
        res = minimize(
            lambda x: loss(x[0]),
            x0=np.log(alpha_hat),
            jac=lambda x: dloss_ridge(x[0]),
            method=optimizer,
            bounds=bounds,
        )

    if res.success:
        dispersion = np.exp(res.x[0])
    else:
        dispersion = np.exp(
            grid_fit_alpha(counts, design_matrix, mu, alpha_hat, min_disp, max_disp)
        )
    diagnostics = {
        "cox_reid_ridge_retry": cox_reid_ridge_retry,
        "cox_reid_ridge_epsilon": 1e-6,
        "converged": bool(res.success),
    }
    if return_diagnostics:
        return dispersion, res.success, diagnostics
    return dispersion, res.success
